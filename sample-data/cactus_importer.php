...
function tm_importer() {
	...
	if ( current_user_can( 'manage_options' ) && isset( $_GET['import_data'] ) ) {	
		...
		$theme_xml = $sample_data_folder . 'university.xml.gz';
		...
		if($menus) {
			foreach($menus as $menu) { // assign menus to theme locations
				if( $menu->name == 'Main Navigation' ) {
					$locations['primary-menus'] = $menu->term_id;
				}
				if( $menu->name == 'Top Nav' ) {
					$locations['secondary-menus'] = $menu->term_id;
				}
			}
		}
		...
		$widgets_json = $sample_data_uri . 'university-widgets.json'; // widgets data file
